# Используем официальный образ Python
FROM python:3.12-slim

# Устанавливаем рабочую директорию
WORKDIR /app

# Устанавливаем системные зависимости
RUN apt-get update && apt-get install -y \
    gcc \
    && rm -rf /var/lib/apt/lists/*

# Копируем файл зависимостей
COPY requirements.txt .

# Устанавливаем Python зависимости
RUN pip install --no-cache-dir -r requirements.txt

# Копируем все файлы проекта
COPY . .

# Создаем директорию для базы данных
RUN mkdir -p /app/data

# Переменные окружения по умолчанию (могут быть переопределены)
ENV PYTHONUNBUFFERED=1
ENV DATABASE_URL=sqlite:///bot_database.db
ENV PORT=8080

# Открываем порт (требуется для облачных платформ)
EXPOSE 8080

# Скрипт инициализации и запуска с улучшенной обработкой ошибок
RUN echo '#!/bin/bash\n\
set -e\n\
\n\
echo "🎉 Запуск PlummyPromo Telegram Bot"\n\
echo "=================================="\n\
\n\
# Проверка переменных окружения\n\
if [ -z "$BOT_TOKEN" ]; then\n\
    echo "❌ ОШИБКА: BOT_TOKEN не установлен!"\n\
    echo "Установите переменную окружения BOT_TOKEN"\n\
    exit 1\n\
fi\n\
\n\
if [ -z "$ADMIN_ID" ]; then\n\
    echo "⚠️  ПРЕДУПРЕЖДЕНИЕ: ADMIN_ID не установлен"\n\
fi\n\
\n\
# Инициализация базы данных если её нет\n\
if [ ! -f "bot_database.db" ]; then\n\
    echo "🔄 Инициализация базы данных..."\n\
    python3 -c "\
import asyncio\n\
from database.database import db\n\
\n\
async def init():\n\
    try:\n\
        await db.init()\n\
        print('\''✅ База данных инициализирована'\'')\n\
    except Exception as e:\n\
        print(f'\''❌ Ошибка инициализации БД: {e}'\'')\n\
        raise\n\
\n\
asyncio.run(init())\n\
" || exit 1\n\
fi\n\
\n\
# Запускаем бота\n\
echo "🚀 Запускаем бота..."\n\
exec python3 -u main.py\n\
' > /app/start.sh && chmod +x /app/start.sh

# Запуск бота
CMD ["/app/start.sh"]

